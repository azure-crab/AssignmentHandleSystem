...
char *cJSONUtils_FindPointerFromObjectTo(cJSON *object,cJSON *target)
{
	...
	if (object==target) return _strdup("");
...
static cJSON *cJSONUtils_PatchDetach(cJSON *object,const char *path)
{
	...
	parentptr=_strdup(path);	childptr=strrchr(parentptr,'/');	if (childptr) *childptr++=0;
...
static int cJSONUtils_ApplyPatch(cJSON *object,cJSON *patch)
{
	...
	parentptr=_strdup(path->valuestring);	childptr=strrchr(parentptr,'/');	if (childptr) *childptr++=0;
	parent=cJSONUtils_GetPointer(object,parentptr);
	cJSONUtils_InplaceDecodePointerString(childptr);
